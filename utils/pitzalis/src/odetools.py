...
def solver (ode_function, interval, protoGen, mapReactions, parameters, divisionTest, environment, coefficient, userCheck):
    ...
    while divisionTest (t, protoAct, parameters):
        ...
        if verbose: 
            if t > seconds:
                print("time:\t%f\t%s"%(t,str(protoAct[0])))
                seconds += intervals
    ...
    #MV
    fmv=open("tempo.txt","w")
    fmv.write(str(t)+"\n")
    fmv.close()
    fmv=open("quantita.txt","r")
    riga=fmv.readline()
    fmv.close()
    fmv=open("quantita.txt","w")
    fmv.write(riga)
    for jm in range(len(riga.split())):
        fmv.write(str(protoAct[jm])+"\t")
    """
    fmv.write("\n")        
    for el in protoAct:
        fmv.write(str(el)+"\t")
...
def simulation (verbose, ecomode, currentTime, environment, parameters, chemicalSpecies, reactions): 
    ...
    #MV
    fmv=open("quantita.txt","w")        
    for el in chemicalSpecies.keys():
        fmv.write(el+"\t")
    fmv.write("\n")    
    fmv.close()
    fmv=open("quantita_T.txt","w")        
    for el in chemicalSpecies.keys():
        fmv.write(el+"\t")
    fmv.write("\n")    
    fmv.close()    
    ...
    try: 
        ...
        for i in range(nIterates):
            ...
            #MV
            if i==0:
                fmv=open("tempi.txt","w")
            else:
                fmv=open("tempi.txt","a")
            if ecomode:
                fmv.write(str(solverTime)+"\n")
            else:
                fmv.write(str(solverTime[-1])+"\n")
            fmv.close()
            
            if i==0:
                fmv=open("quantita_T.txt","a")
            else:
                fmv=open("quantita_T.txt","a")
            if ecomode:
                #fmv.write(str(solverTime)+"\n")
                for j in range(len(loadedSpecies)):
                    fmv.write(str(y_sol[j])+"\t")
                fmv.write("\n") 
                """
                for el in y_sol:
                    fmv.write(str(el)+"\t")
                ...
                for jm in range(len(loadedSpecies)):
                    fmv.write(str(y_sol[-1][jm])+"\t")
                fmv.write("\n")
                """
                for el in y_sol[-1]:
                    fmv.write(str(el)+"\t")
            ...
            #print("SONOQUISONOQUISONOQUISONOQUISONOQUISONOQUISONOQUISONOQUISONOQUI")
            #print(solverTime[-1])
            #MV

            #print("solverTime:",solverTime)
            #print("t_end:",t_end)
            if isinstance(solverTime, list):
                if solverTime[-1] >= t_end:
                    if not verbose:
                        progress_bar.close()
                    workbook.close()            
                    return (time_, mat)#quit()
            else:
                if solverTime >= t_end:
                    if not verbose:
                        progress_bar.close()
