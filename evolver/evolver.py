...
class Evolver:
    ...
    def copyChemistryFiles(self):
        ...
        copyFile(chemistryFile, EVOLVER_CHEMISTRY_FILE)
        copyFile(rulesFile, EVOLVER_CHEMISTRY_RULES_FILE)

    def generateRAFfile(self):
        ...
        copyFile(EVOLVER_CHEMISTRY_FILE, venturiInputPath)
        ...
        if os.listdir(venturiOutputPath):
            outPath = os.path.join(EVOLVER_RAF_FILE)
    ...
    def writeContainerSpeciesFile(self, initialize=False):
        if initialize:
            ...
            catalysts = self.getContainerCatalysts()
        concExtFC = "10"
        mdiffFC = "1.00e-18"
        alfa = "1.00e-02"
        i = 0
        max_length = max(len(c) for c in catalysts)
    ...
    def addContainerSpecies(self,initialize=False):
        self.writeContainerSpeciesFile(initialize=initialize)
        copyFile(EVOLVER_RAF_FILE, PITZALIS_CONTAINERS_SPECIES_INPUT)
        runPitzalisContainers(PITZALIS)
        copyFile(PITZALIS_CONTAINERS_SPECIES_OUTPUT, EVOLVER_RAF_FILE)
        formatRAFFile(EVOLVER_RAF_FILE)

    def pitzalisSimulator(self):
        copyFile(EVOLVER_RAF_FILE, PITZALIS_INPUT)
    ...
    def modifyMutatedChemistryFile(self, mutatedChemistryFile):
        with open(mutatedChemistryFile, "r") as f:
            lines = f.readlines()
        with open(mutatedChemistryFile, "w") as f:
            for line in lines:
                if not NUMERIC_START.match(line) and not "+" in line and line.strip() and not line.startswith("#"):
                    line = line.split()
                    line[1] = str(self.getSingleConcentration(line[0]))
                    f.write(" ".join(line) + "\n")
                else:
                    f.write(line)
        formatRAFFile(mutatedChemistryFile)

    def mutateParent(self):
        shutil.rmtree(MUTATOR_INPUT, ignore_errors=True)
        os.makedirs(MUTATOR_INPUT, exist_ok=True)
        shutil.copyfile(EVOLVER_CHEMISTRY_FILE, MUTATOR_CHEMISTRY_FILE)
        ...
        runMutator()
        formatFileForVenturi(MUTATOR_OUTPUT_FILE)
        self.modifyMutatedChemistryFile(MUTATOR_OUTPUT_FILE)
        shutil.copyfile(MUTATOR_OUTPUT_FILE, EVOLVER_RAF_FILE)
    
    def evolve(self):
        i = 0
        while i<self.getParameter("numberOfEvolutions"):
            self.addContainerSpecies(initialize=(i==0))
            time = self.pitzalisSimulator()
            if time > (self.getPreviousTime() * (self.getParameter("timeTolerance")/100)):
                exit()
